...
namespace Factory_Project
{
    ...
    class Factory
    {
        ...
        public void UpdateTotalRevenue(decimal totalRevenue)
        {
        ...
        {
            // Ensure the products collection is not empty before proceeding.
            if (products.Count == 0)
            {
                return null;
            }

            Product maxPriceProduct = products[0]; // Assume the first product has the maximum price.
        ...
        public void GenerateData()
        {
            ...
            // Create 10 real products manually
            Product product1 = new Product("iPhone 12", "1234567891", 5, DateTime.Now.AddDays(-10), 899.99m);
            Product product2 = new Product("Samsung Galaxy S21", "1234567892", 8, DateTime.Now.AddDays(-155), 799.99m);
            Product product3 = new Product("Sony PlayStation 5", "1234567893", 3, DateTime.Now.AddDays(-250), 499.99m);
            Product product4 = new Product("Nike Air Max 270", "1234567894", 10, DateTime.Now.AddDays(-55), 149.99m);
            Product product5 = new Product("Canon EOS R5", "1234567895", 2, DateTime.Now.AddDays(-7), 2499.99m);
            Product product6 = new Product("Apple AirPods Pro", "1234567896", 6, DateTime.Now.AddDays(-152), 199.99m);
            Product product7 = new Product("Dell XPS 15", "1234567897", 4, DateTime.Now.AddDays(-58), 1799.99m);
            Product product8 = new Product("Nintendo Switch", "1234567898", 7, DateTime.Now.AddDays(-43), 299.99m);
            Product product9 = new Product("Adidas Ultraboost", "1234567899", 9, DateTime.Now.AddDays(-676), 129.99m);
            Product product10 = new Product("LG OLED CX Series", "1234567810", 1, DateTime.Now.AddDays(-9), 1499.99m);
        ...
        public void LoadData()
        {
            try
            {
                ...
                {
                    string line;
                    bool isReadingTotalRevenue = false;
                    bool isReadingProducts = false;
                    bool isReadingEmployees = false;

                    while ((line = reader.ReadLine()) != null)
                    {
                        if (line == "TotalRevenue:4499.95.")
                        {
                            isReadingTotalRevenue = true;
                        ...
                        if (isReadingTotalRevenue)
                        {
                            string totalRevenueLine = line.Trim();
                            if (totalRevenueLine.StartsWith("TotalRevenue:"))
                            {
                                decimal totalRevenue = decimal.Parse(totalRevenueLine.Substring("TotalRevenue:".Length));
                                TotalRevenue = totalRevenue;
                            }
                            else
                            {
                                DisplayErrorMSg("Invalid total revenue data format.");
                            }
                        }
                        else if (isReadingProducts)
                        {
                            ...
                            if (productData.Length == 5)
                            {
                                ...
                                decimal price = decimal.Parse(productData[4].Trim());

                                Product product = new Product(productName, barcode, count, dateAdded, price);
                                products.Add(product);
                            }
                            else
                            {
                                DisplayErrorMSg("Invalid product data format.");
                            }
                        }
                        else if (isReadingEmployees)
                        {
        ...
        public void SaveData()
        {
            try
            {
                ...
                {
                    writer.WriteLine($"TotalRevenue:{TotalRevenue}.");
                    writer.WriteLine();
                    ...
                    foreach (Product product in products)
                    {
                        ...
                        writer.WriteLine($"{product.Name}, {product.Barcode}, {product.Count}, {product.DateAdded.ToString("yyyy-MM-dd")}, {product.Price}");
                    }
